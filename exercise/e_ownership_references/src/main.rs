...
fn bedazzle(mut new_str: &mut String) {
    *new_str = "sparkly".to_string()
}

fn inspect(str: &str) -> bool {
    println!("{}", str.ends_with("s"));
    str.ends_with("s")
}

fn change(mut new_str: &mut String) -> String {
    if inspect(&new_str) {
        new_str.to_string()
    } else {
        new_str.push_str("s");
...
fn eat(new_str: String) -> bool {
    if new_str.starts_with("b") && new_str.contains("a") {
        true
    } else {
        false
    }
